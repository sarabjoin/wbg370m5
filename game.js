...
$("#playerBody").html("<span class='value'>" + $("#player")
[0].player.value + "</span><br />(<span class='symbol'>" + 
symbols[symbolIndex] + "</span>) <span class='number'>" + 
$("#player")[0].player.number + "</span>");


$.playground().registerCallback(function(){
    $("#background1").x(($("#background1").x() - 
    farParallaxSpeed - PLAYGROUND_WIDTH) % (-2 * 
    PLAYGROUND_WIDTH) + PLAYGROUND_WIDTH);
    $("#background2").x(($("#background2").x() - 
    farParallaxSpeed - PLAYGROUND_WIDTH) % (-2 * 
    PLAYGROUND_WIDTH) + PLAYGROUND_WIDTH);
    $("#background3").x(($("#background3").x() - 
    closeParallaxSpeed - PLAYGROUND_WIDTH) % (-2 * 
    PLAYGROUND_WIDTH) + PLAYGROUND_WIDTH);
    $("#background4").x(($("#background4").x() - 
    closeParallaxSpeed - PLAYGROUND_WIDTH) % (-2 * 
    PLAYGROUND_WIDTH) + PLAYGROUND_WIDTH);
    $(".enemy").each(function(){
        this.enemy.update();
        if(($(this).x() + enemyWidth) < 0){
            ...
            var collided = $(this).collision("#playerBody,."
            + $.gQ.groupCssClass);
            if(collided.length > 0){
                $("#player")[0].player.value += $(this)
                [0].enemy.value;
                $("#player")[0].player.number = $(this)
                [0].enemy.value;
                $("#player .value").html($("#player")
                [0].player.value);
                $("#player .number").html($("#player")
                [0].player.number);
                $(this).remove();
                ...
                if ($("#player")[0].player.value >= 100){
                    ...
                    $("div").eq(2).html("<h1 style='text-align:center;'>You win!</h1>");
                }
                if ($("#player")[0].player.value <= -100){
                    $.playground().pauseGame();
                    $("div").eq(2).html("<h1 style='text-align:center;'>You Lose!</h1>");
                }
        }
    }
});
...
$.playground().registerCallback(function(){
    ...
    $("#enemies").addSprite(name, {
        ...
        posy: Math.random()*PLAYGROUND_HEIGHT*0.9, 
        width: enemyWidth, 
        height: enemyHeight
    });
